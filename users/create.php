...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $fullName === '' || strlen($password) < 6) {
        set_flash('error', 'Username, full name, and a password of at least 6 characters are required.');
        redirect('users/create.php');
    }
    try {
        $hash = password_hash($password, PASSWORD_DEFAULT);
        $pdo->prepare('INSERT INTO users (username, password, full_name, role, status) VALUES (?,?,?,?,?)')
            ->execute([$username, $hash, $fullName, $role, $status]);
